...
subroutine calc_rad_accel_from_ptmass(npart,i,dx,dy,dz,Lstar,Mstar,rstar,vwind,alpha_rad,fextx,fexty,fextz,tau)
 ...
 elseif (do_condensation) then
    if (itau_alloc == 1) then
       call get_radiative_acceleration_from_star(r,dx,dy,dz,Mstar,Lstar,rstar,vwind,&
               condensation(ickappa,i),alpha_rad,ax,ay,az,condensation(icalpha,i),tau(i))
    else
       call get_radiative_acceleration_from_star(r,dx,dy,dz,Mstar,Lstar,rstar,vwind,&
               condensation(ickappa,i),alpha_rad,ax,ay,az,condensation(icalpha,i))
    endif
 else
...
subroutine get_dust_temperature(npart,xyzh,eos_vars,nptmass,xyzmh_ptmass,dust_temp)
 use part,      only:tau,tau_lucy,idkappa,nucleation,ickappa,condensation,rho
 ...
 if (iget_tdust == 4) then
    ...
    if (idust_opacity == 2) then
       call get_all_tau(npart, nptmass, xyzmh_ptmass, xyzh, rho, nucleation(:,idkappa), iray_resolution, .false., tau_lucy)
    elseif (idust_opacity == 3) then
       call get_all_tau(npart, nptmass, xyzmh_ptmass, xyzh, rho, condensation(:,ickappa), iray_resolution, .false., tau_lucy)
    else
 ...
 elseif (itau_alloc == 1) then
    ! update tau
    if (idust_opacity == 2) then
       call get_all_tau(npart, nptmass, xyzmh_ptmass, xyzh, rho, nucleation(:,idkappa), iray_resolution, .true., tau)
    elseif (idust_opacity == 3) then
       call get_all_tau(npart, nptmass, xyzmh_ptmass, xyzh, rho, condensation(:,ickappa), iray_resolution, .true., tau)
    else
       call get_all_tau(npart, nptmass, xyzmh_ptmass, xyzh, rho, calc_kappa_bowen(dust_temp(1:npart)),&
