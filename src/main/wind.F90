...
!#define CALC_HYDRO_THEN_CHEM
 ...
 real :: wind_gamma
 real, allocatable, target, public :: trvurho_1D(:,:),trvurho_1D2(:,:),dust_array_1D(:,:)
...
! all quantities in cgs
 ...
 use ptmass_radiation,  only:iget_tdust,tdust_exp,isink_radiation
 use physcon,           only:pi,Rg
 ...
 if (idust_opacity == 2) then
    ...
    state%dust_array(idalpha) = state%alpha_Edd+params%alpha_rad
 elseif (idust_opacity == 3) then
    !state%Tg   = state%u*(state%gamma-1.)/Rg*state%mu
...
!
!-----------------------------------------------------------------------
subroutine interp_wind_profile(time,local_time,r,v,u,rho,e,GM,fdone,isink,dust_array)
...
! all quantities are in cgs
 ...
 real :: r_incr,v_incr,T_incr,mu_incr,gamma_incr,r_base,v_base,T_base,mu_base,gamma_base,eps,time_base,time_incr
