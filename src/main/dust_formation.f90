...
!all quantities in cgs
 ...
 if (T > 450.) then
    ...
    if (S > Scrit) then
       ...
       if (any(JKmuS(idK0:idK3) > tiny(0.0d0))) then
          call calc_nucleation(T, pC, pC2, 0.0, pC2H, pC2H2, S, JstarS, taustar, taugr)
          adot = 1. / 3. / taugr    ! Equation 28 in Gauger 1990
          call evap_shift_remove(JKmuS(idK0:idK3), dt, adot, K_new(0:3))
          Jstar_new = 0.0
          if (any(K_new(0:3) > 1.0)) then
             print*, '[S-dust_formation] Evaporation failed: K moments =', K_new(0:3)
          elseif (any(K_new(0:3) < 0.0d0)) then
             print*, '[S-dust_formation] All dust evaporated'
          elseif (any(isnan(K_new(0:3)))) then
             print*, '[S-dust_formation] NaNs in K_new after evaporation:', K_new(0:3)
             stop
          endif
       else
          Jstar_new = 0.0
...
! Simplified low-temperature chemistry: all hydrogen in H2 molecules, all O in CO
 ...
 if (any(JKmuS(idK0:idK3) < 0.0)) then
    print*, '[S-dust_formation] Negative moments in evolve_chem: K0=',JKmuS(idK0),&
          ' K1=',JKmuS(idK1),' K2=',JKmuS(idK2),' K3=',JKmuS(idK3)
    JKmuS(idK0:idK3) = max(JKmuS(idK0:idK3), 0.0)
 elseif (any(isnan(JKmuS(idK0:idK3)))) then
    print*, '[S-dust_formation] NaNs in evolve_chem: K0=',JKmuS(idK0),&
          ' K1=',JKmuS(idK1),' K2=',JKmuS(idK2),' K3=',JKmuS(idK3)
    stop
 endif
...
! Use this simpler version as an alternative if needed
! pure elemental function phi(z) result(cdf)
!   implicit none
!   real(8), intent(in) :: z
!   real(8) :: cdf
!   real(8), parameter :: zmax = 38.0d0
!   real(8), parameter :: s2 = sqrt(2.0d0)
...
subroutine evap_shift_remove(M, dt, adot, Mf)
 ...
 if (any(Mf(:) /= Mf(:))) then
    print*, '[S-dust_formation] NaNs in evap_shift_remove: M0i=',M0i,' M1i=',M1i,' M2i=',M2i,' M3i=',M3i,&
          ' delta_a=',delta_a,' M0f=',M0f,' M1f=',M1f,' M2f=',M2f,' M3f=',M3f
 endif
