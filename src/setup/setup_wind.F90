...
subroutine setpart(id,npart,npartoftype,xyzh,massoftype,vxyzu,polyk,gamma,hfact,time,fileprefix)
 ...
 use set_dust_options, only: set_dust_default_options,smincgs,smaxcgs,sindex,&
      dust_method,ndustsmallinp,ndusttypesinp,grainsizeinp
#endif
...
!
 ...
 if (iexist) then
    ...
    if (id==master) call write_setupfile(iunit,filename)
    if (ierr /= 0) then
       stop
    endif
 ...
 elseif (id==master) then
    ...
    call write_setupfile(iunit,filename)
...
!        print "(a,g10.3,a)",'                       Accretion Radius 2b: ', racc2b!, trim(dist_unit)
       ...
       print *,'Sink particles summary (wind is launched from sink #1)'
...
subroutine setup_interactive()
 ...
 use io,        only:fatal
#ifdef DUST
 use set_dust_options, only:set_dust_interactively,dust_method
#endif
 integer :: ichoice
 ...
 !binary or single star case
    ...
    if (icompanion_star == 1) then
        ...
        print "(a)",' 2: Mass = 1.0 Msun, accretion radius = 0.00465 au',&
             ' 1: Mass = 1.0 Msun, accretion radius = 0.1 au',&
             ' 0: custom'
...
!+
!----------------------------------------------------------------
subroutine write_setupfile(iunit,filename)
...
subroutine read_setupfile(filename,ierr)
 ...
 use set_dust_options, only:read_dust_setup_options,ndusttypesinp,dust_method,ilimitdustfluxinp
#endif
 character(len=*), intent(in)  :: filename
